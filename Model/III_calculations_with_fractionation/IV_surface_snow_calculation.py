# This file was used to calculate the surface snow isotopic composition for the 'with fractionation' case

# This is a function file which is called by 'IV_surface_snow_multiprocessing.py'
...
def isotope(longitude_start, longitude_end, change_name, depth):
    ...
    for a in range(0, time):
        ...
        alpha_H = np.exp(0.2133 - (203.10 / D['skt'][a]) + (48888 / (D['skt'][a]) ** 2))  # Hydrogen fractionation factor for surface temperature
        alpha_O = np.exp(0.0831 - (49.192 / D['skt'][a]) + (8312.5 / (D['skt'][a]) ** 2))  # Oxygen fractionation factor for surface temperature
            ...
            for c in range(longitude_start, longitude_end):
                ...
                if lsm_only_antarctica[b,c] == 0:
                    ...
                    if LE <= 0:
                        ...
                        R_H_subl[-1] = np.array(R_H_snow_layer[c + (b * 1440)][-1]) / alpha_H[b, c]
                        R_O_subl[-1] = np.array(R_O_snow_layer[c + (b * 1440)][-1]) / alpha_O[b, c]
                        ...
                        R_H_depo = np.append(R_H_depo, R_H_lit_air * alpha_H[b, c])
                        R_O_depo = np.append(R_O_depo, R_O_lit_air * alpha_O[b, c])
    ...
    file_save = 'surface_snow_isotopic_composition' + str(change_name)
